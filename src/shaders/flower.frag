...
// Simplified snoise for smaller footprint if needed, but keeping original logic for consistency
...
float get_dot_shape(vec2 dist, float radius_max, float radius_line) {
    return 1. - smoothstep(radius_line * radius_max, max(radius_max, 0.00001), dot(dist, dist) * 4.0);
}
...
void main() {
    ...
    vec2 uv = vUv;
    // Use vertex-calculated noise for base jitter (P2 Optimization)
    uv.y += 0.00005 + 0.00007 * vNoise; 
    ...
    float base_radius = (.02 + .2 * u_stop_randomizer.y) * u_scale;
